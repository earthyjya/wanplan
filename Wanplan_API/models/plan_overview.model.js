...
Plan_overview.updateById = (id, plan_overview, result) => {
  sql.query(
    "UPDATE plan_overview SET plan_name = ?, user_id = ?, city_id = ?, duration = ?, plan_style = ?, plan_description = ?, star_rating = ? WHERE plan_id = ?",
    [plan_overview.plan_name, plan_overview.user_id, plan_overview.city_id, plan_overview.duration, plan_overview.plan_style, plan_overview.plan_description, plan_overview.star_rating, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
      ...
      if (res.affectedRows == 0) {
        // not found plan_overview with the id
        result({ kind: "not_found" }, null);
        return;
      }
