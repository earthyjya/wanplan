...
exports.deleteUser = (req, res) => {
  Plan_detail.removeUser(req.params.userId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found plan_detail of user with user_id ${req.params.userId}.`
        });
      } else {
        res.status(500).send({
          message: "Could not delete plan_detail of user with user_id " + req.params.userId
        });
      }
    } else res.send({ message: `Plan_detail of user with user_id was deleted successfully!` });
  });
};
