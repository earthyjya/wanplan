...
Plan_detail.updateByIdOne = (id, day, order, plan_detail, result) => {
  sql.query(
    "UPDATE plan_detail SET plan_id = ?, day = ?, attraction_order = ?, start_time = ?, end_time = ?, time_spend = ?, attraction_id = ? WHERE plan_id = ? AND day = ? AND attraction_order = ?",
    [plan_detail.plan_id, plan_detail.day, plan_detail.attraction_order, plan_detail.start_time, plan_detail.end_time, plan_detail.time_spend, plan_detail.attraction_id, id, day, order],
	(err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
